...
- **退出 tmux 会话（保持后台运行）：** 按下 `Ctrl+b`（前缀键），然后按 `d` 键。
...
- **切换窗口：**
  - 按下 `Ctrl+b`，然后按 `n`（下一个窗口）。
  - 按下 `Ctrl+b`，然后按 `p`（上一个窗口）。
  - 按下 `Ctrl+b`，然后按数字键（如 `0`、`1`）切换到指定窗口。
- **重命名窗口：** 按下 `Ctrl+b`，然后按 `,`，输入新窗口名。
- **关闭当前窗口：** 输入 `exit`，或者直接关闭窗口所在的终端任务。
...
- **水平分屏：** 按下 `Ctrl+b`，然后按 `%`。
- **垂直分屏：** 按下 `Ctrl+b`，然后按 `"`。
- **在面板间切换：** 按下 `Ctrl+b`，然后按箭头键（←、→、↑、↓）。
- **调整面板大小：** 按下 `Ctrl+b`，然后：
  - 按 `Ctrl+←` 或 `Ctrl+→` 调整水平宽度。
  - 按 `Ctrl+↑` 或 `Ctrl+↓` 调整垂直高度。
- **关闭当前面板：** 输入 `exit`，或按 `Ctrl+d`。
...
- **在同一个会话中切换窗口：** 按下 `Ctrl+b`，然后按 `w`，选择窗口。
...
可以通过编辑 `~/.tmux.conf` 配置文件自定义快捷键和行为。例如：
...
`$()` 是命令替换的语法，它允许你在一个命令中嵌入另一个命令，并将嵌套命令的输出作为结果返回。
...
这会执行 `which ranger` 命令，返回 `ranger` 可执行文件的路径，然后将这个路径作为结果返回，替换掉 `$(which ranger)` 这个部分。

### 2. **变量替换** (`$VAR`)

在 Shell 中，`$` 也用来引用变量的值。例如，如果你定义了一个变量 `VAR`，你可以用 `$VAR` 来获取它的值：
...
当你使用 `$(...)` 命令替换时，它会先执行里面的命令，然后用结果替代 `$(...)` 部分。这是 Shell 中常见的一种技巧，用来动态获取命令输出作为路径、参数等。
...
```bash
cd $(dirname $(which ranger))/..
```

- `$(which ranger)` 会执行 `which ranger` 命令并返回 `ranger` 的路径。
- `$(dirname $(which ranger))` 会通过 `dirname` 命令获取 `ranger` 所在的目录。
- 最终，`/..` 会将当前目录向上移动一级。
